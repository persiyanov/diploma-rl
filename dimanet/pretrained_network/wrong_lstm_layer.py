"""
implements the lstm with wrong layer order as in DaNet by 13.09

Copies https://github.com/yandexdataschool/AgentNet/blob/master/agentnet/memory/rnn.py#L219 with only a single difference - output gate is applied AFTER tanh
...
class WrongLSTMLayer(MergeLayer):
    r"""
    see regular LSTM docstring
    """
    def __init__(self, incoming, num_units,
        ...
        super(WrongLSTMLayer, self).__init__(incomings, **kwargs)
        ...
        self.return_cell_and_hid=return_cell_and_hid
    ...
    def get_output_shape_for(self, input_shapes):
        ...
        if self.only_return_final:
            shape =input_shape[0], self.num_units
        # Otherwise, the shape will be (n_batch, n_steps, num_units)
        else:
            shape = input_shape[0], input_shape[1], self.num_units
            
        if self.return_cell_and_hid:
            shape+=(2,)
        return shape
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step(input_n, cell_previous, hid_previous, *args):
            ...
            hid = self.nonlinearity(cell*outgate)#!!!CHANGES
        ...
        if self.return_cell_and_hid:
            return T.stack([cell_out,hid_out],-1)
        else:
            return hid_out
